...
TEST_F(ErrorTest, BasicErrorCreation)
{
    ...
    EXPECT_EQ(error.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::CONTAINER_NOT_FOUND);
    EXPECT_STREQ(error.what(),
                 "[docker-cpp 1000] Container not found: Container with ID 'test' not found");
}

TEST_F(ErrorTest, docker_cpp::ErrorCodeConversion)
{
    // Test error code to string conversion
    docker_cpp::ContainerError error1(docker_cpp::ErrorCode::CONTAINER_NOT_FOUND, "Test message");
    EXPECT_EQ(error1.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::CONTAINER_NOT_FOUND);

    docker_cpp::ContainerError error2(docker_cpp::ErrorCode::NAMESPACE_CREATION_FAILED, "Namespace creation failed");
    EXPECT_EQ(error2.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::NAMESPACE_CREATION_FAILED);
}
...
TEST_F(ErrorTest, ErrorCopyAndMove)
{
    ...
    EXPECT_EQ(copied.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::IMAGE_NOT_FOUND);
    ...
    EXPECT_EQ(moved.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::IMAGE_NOT_FOUND);
}
...
TEST_F(ErrorTest, ErrorWithSystemError)
{
    ...
    EXPECT_EQ(container_error.getdocker_cpp::ErrorCode(), docker_cpp::ErrorCode::SYSTEM_ERROR);
...
TEST_F(ErrorTest, ErrorCategory)
{
    ...
    const auto& category = getdocker_cpp::ContainerErrorCategory();
...
TEST_F(ErrorTest, docker_cpp::ErrorCodeValues)
{
